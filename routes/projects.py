...
@bp.route('/get/byProjectId/{project_id}', methods=['GET'])
def get(project_id):
    project_db.Query_One({'_id': bson.ObjectId(project_id)})


@bp.route('/get/byUserId/{user_id}', methods=['GET'])
def get(user_id):
    results = project_db.Query({ 'user_ids': { '$in': [user_id] } })
    return jsonify({'success': True, 'results': results})


@bp.route('/update/byProjectId/{project_id}', methods=['POST'])
...
@bp.route('/delete/byProjectId/{project_id}', methods=['POST'])
